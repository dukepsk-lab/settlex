...
from .scraper import get_macro_features, fetch_nvdr_data
...
def load_symbol(
    ...
    if synthetic:
        ...
        # Merge Scraper Data (Macro & NVDR)
        today_str = dt.date.today().strftime('%Y-%m-%d')
        macros = get_macro_features(today_str)
        nvdr_df = fetch_nvdr_data()
        
        # Add Macro Columns (For historical rows, we could fetch historical macro, but here we just pad zeros for now)
        if 'THB_USD' not in df.columns:
            df['THB_USD'] = 0.0
            df['US_10Y_YIELD'] = 0.0
        
        # Update the latest row with today's macros
        if not df.empty:
            df.loc[df.index[-1], 'THB_USD'] = macros.get('THB_USD', 0.0)
            df.loc[df.index[-1], 'US_10Y_YIELD'] = macros.get('US_10Y_YIELD', 0.0)
            
        # Add NVDR Columns
        if 'nvdr_net_value' not in df.columns:
            df['nvdr_net_value'] = 0.0
            df['foreign_net_value'] = 0.0
            
        if nvdr_df is not None and not nvdr_df.empty:
            sym_nvdr = nvdr_df[nvdr_df['symbol'] == symbol]
            if not sym_nvdr.empty:
                df.loc[df.index[-1], 'nvdr_net_value'] = float(sym_nvdr['nvdr_net_value'].iloc[0])
                df.loc[df.index[-1], 'foreign_net_value'] = float(sym_nvdr['foreign_net_value'].iloc[0])
